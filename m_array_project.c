...
int main() {
	...
	while (1) {
		...
		//같은 동물인 경우
		if (checkAnimal[firstSelect_x][firstSelect_y] == 0 &&  //첫 번째 뽑은 카드가 뒤집히지 않은 카드
			checkAnimal[secondSelect_x][secondSelect_y] == 0 && //두 번째 뽑은 카드가 뒤집히지 않은 카드
			arrayAnimal[firstSelect_x][firstSelect_y] == arrayAnimal[secondSelect_x][secondSelect_y]) {
			//첫 번째 뽑은 카드와 두 번째 뽑은 카드가 같은 동물인지
...
void shuffleAnimal() {
	...
	//□□□□□
	for(int i = 0; i < 10; i++) { 
		//빈 카드를 2개를 뽑아 동일한 번호를 새기는 것을 10번 반복
		for(int j = 0; j < 2; j++) {
...
void printQuestion() {
	printf("\n\n(문제)\n");
	int seq = 0;
						//seq							//checkAnimal => 1은 맞춘것, 0은 아직 발견 못한 것.
	//□□□□□    0	1	2	3	 4					0 0 0 0 0
